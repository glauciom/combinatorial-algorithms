...
/**
 * @author Glaucio Melo Classe que retorna um vetor permutado, dado o seu vetor
 *         de deslocamento.
 */
public class SerialPermutation {
	...
	public SerialPermutation(int[] offset, int n) {
		this.offset = offset;
		this.n = n;
		init();
	}
	...
	private int[] blindAlgorithm() {
		elements[n - 1] = 1;
		for (i = 1; i < n; i++)
			elements[n - 1 - offset[i - 1]] = elements[n - 1 - offset[i - 1]] == 0 ? i + 1
					: push();
		for (i = 0; i < n; i++)
